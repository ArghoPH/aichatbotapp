      ...
      <header
        ...
        <div class="flex items-center gap-3">
          <button type="button" @click="clearChatHistory"
            class="flex items-center gap-2 rounded-2xl border border-blue-500/20 bg-blue-500/10 px-4 py-2 text-sm text-blue-300 transition hover:bg-blue-500/20">
        ...
        <aside class="space-y-5 lg:col-span-1 min-h-0 flex flex-col h-full overflow-hidden">
          ...
          <div class="flex-1 min-h-0 overflow-y-auto custom-scrollbar space-y-5">
            ...
            <div class="rounded-3xl border border-white/10 bg-white/10 p-5 shadow-2xl backdrop-blur-xl">
              <h2 class="mb-4 text-sm font-bold tracking-wider text-slate-400 uppercase">
              ...
              <div v-else class="space-y-2">
                <button @click="loadHistory"
                  class="w-full text-left rounded-xl bg-white/5 hover:bg-white/10 px-3 py-2.5 text-xs transition border border-white/5 flex items-center gap-2">
                  <i class="fa-regular fa-message text-blue-400 text-sm"></i>
                  <div class="truncate flex-1">
                    <p class="font-medium truncate text-slate-200">Current Session</p>
            ...
            <div class="rounded-3xl border border-white/10 bg-white/10 p-5 shadow-2xl backdrop-blur-xl">
              ...
              <div class="space-y-1 text-xs text-slate-400 mt-3">
                <p><span class="text-slate-300 font-medium">Backend:</span> ASP.NET Core</p>
                <p><span class="text-slate-300 font-medium">Database:</span> SQL Server</p>
                <p><span class="text-slate-300 font-medium">Core AI:</span> Gemini API</p>
              </div>
            </div>
...
// Clear / New Chat Functionality (Abar add kora holo)
async function clearChatHistory() {
  const confirmed = confirm('Are you sure you want to clear all chat history and start a new chat?')
  if (!confirmed) {
    return
  }
