<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">
    <div class="mx-auto flex min-h-screen max-w-7xl flex-col px-4 py-5">
      ...
      <main class="grid flex-1 min-h-0 grid-cols-1 gap-5 lg:grid-cols-4">

        <!-- Chat Area -->
        <section
          class="flex h-[calc(100vh-150px)] min-h-0 flex-col overflow-hidden rounded-3xl border border-white/10 bg-white/10 shadow-2xl backdrop-blur-xl lg:col-span-3">
          <!-- Chat Topbar -->
            ...
            <div v-else class="space-y-3">
              <div v-for="provider in providerStatuses" :key="provider.name"
                ...
                <div class="flex items-center justify-between gap-3">
                  ...
                  <span class="rounded-full border px-3 py-1 text-xs" :class="getProviderStatusClass(provider.status)">
                    {{ provider.status }}
                  </span>
                </div>
